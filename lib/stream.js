...
module.exports = function(template,key,env) {
  ...
  input_stream.on('pipe',function(src) {
    ...
    if ( ! src._readableState || src._readableState.objectMode ) {
      ...
      transformed_stream = stream_transform_object(src,key ? template[key] : template,env).pipe(new ContextWriter(template,key));
    } else {
      src.unpipe(input_stream);
      if (key) {
        input_stream._readableState.objectMode = false;
      }
      transformed_stream = stream_transform(src,key ? template[key] : template,env).pipe(new ContextWriter(template,key));
    }
