...
function ContextWriter(template,key,options) {
  // allow use without new
  if (!(this instanceof ContextWriter)) {
    return new ContextWriter(template,key,options);
  }
  if ( ! options ) {
    options = {};
  }
...
ContextWriter.prototype._flush = function (cb) {
  if (this.key) {
    ...
    Object.keys(template).filter(function(key) {
      ...
      self.push('\t"'+key+'":'+JSON.stringify(template[key])+',\n');
    });
    this.push('}\n');
  }
  cb();
...
var stream_transform_object_array = function(stream,template) {
  var selector = convert_selector(template[0]);
  return stream.pipe(new JSONFilter(selector.filter)).pipe(new ObjectTransform(template[1]));
};
...
var stream_transform_object_collect = function(stream, template) {
  var selectors = Object.keys(template);
...
var stream_transform_object = function(stream,template) {
  if (Array.isArray(template)) {
    return stream_transform_object_array(stream,template);
  }
  return stream_transform_object_collect(stream,template);
};
...
module.exports = function(template,key) {
  ...
  input_stream.on('pipe',function(src) {
    ...
    if ( ! src._readableState || src._readableState.objectMode ) {
      ...
      transformed_stream = stream_transform_object(src,key ? template[key] : template).pipe(new ContextWriter(template,key));
    } else {
      src.unpipe(input_stream);
      if (key) {
        input_stream._readableState.objectMode = false;
      }
      transformed_stream = stream_transform(src,key ? template[key] : template).pipe(new ContextWriter(template,key));
    }
