...
function ContextWriter(template,key,isArray,options) {
  // allow use without new
  if (!(this instanceof ContextWriter)) {
    return new ContextWriter(template,key,isArray,options);
  }
  if ( ! options ) {
    options = {};
  }
...
ContextWriter.prototype._flush = function (cb) {
  if (this.key) {
    ...
    remaining_keys.forEach(function(key,idx) {
      self.push('\t"'+key+'":'+JSON.stringify(template[key])+( idx < (remaining_keys.length - 1) ? ',' : '' )+'\n');
    });
    this.push('}\n');
  }
  cb();
...
module.exports = function(template,key,env) {
  ...
  input_stream.on('pipe',function(src) {
    ...
    if ( ! src._readableState || src._readableState.objectMode ) {
      ...
      transformed_stream = stream_transform_object(src,key ? template[key] : template,env).pipe(new ContextWriter(template,key,Array.isArray( key ? template[key] : template )));
    } else {
      src.unpipe(input_stream);
      if (key) {
        input_stream._readableState.objectMode = false;
      }
      transformed_stream = stream_transform(src,key ? template[key] : template,env).pipe(new ContextWriter(template,key,Array.isArray( key ? template[key] : template )));
    }
