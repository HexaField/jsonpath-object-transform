...
function withEnv(env: Record<string, any>, actions: () => void) {
  ...
  try { actions(); } finally { (withEnv as any)._env = {}; }
}
...
function evalExpressionWithResultContainer(src: string, ctx: any, env: Record<string, any>): any {
  ...
  Object.keys(env || {}).forEach((k) => {
    ...
    if (typeof fn === 'function') {
      ...
      wrappedEnv[k] = function(...args: any[]) {
        if (fn.length >= 2) { return fn(container, ...args); }
        return fn(...args);
      };
    } else {
      wrappedEnv[k] = env[k];
  ...
  const m = /^['\"](.*)['\"]$/.exec(keyStr);
  if (m && typeof m[1] === 'string') { keyStr = m[1]; }
  ...
  if (subscriptMatch) {
    ...
    baseVals.forEach((val) => {
      ...
      contexts.forEach((ctx) => {
        ...
        let keyVal: any;
        try { keyVal = evalExpression(expr, ctx, env); } catch (_) { keyVal = undefined; }
        ...
        if (keyStr !== undefined && keyStr !== null) {
          if (Array.isArray(ctx)) {
            ...
            if (!Number.isNaN(idx) && typeof ctx[idx] !== 'undefined') { out.push(ctx[idx]); return; }
          } else if (ctx && typeof ctx === 'object') {
            const k = String(keyStr);
            if (Object.prototype.hasOwnProperty.call(ctx, k)) { out.push(ctx[k]); return; }
          }
        }
        ...
        if (looksLikeContainerCall) {
          try {
            ...
            if (typeof res !== 'undefined') { out.push(res); return; }
          } catch (e) { /* ignore and fall back to raw eval */ }
        }
...
function jpPaths(json: any, pathStr: string): string[] {
  return JSONPath({ path: pathStr, json, resultType: 'path', wrap: true }) as string[];
}

function parentPath(pathStr: string): string {
  // remove last [..] or .prop
...
let walk: (data: any, path: any, result: AnyObject, key?: any) => void;

function typeOf(obj: any): 'array' | 'object' | 'number' | 'string' | 'boolean' | 'function' | 'undefined' | 'bigint' | 'symbol' {
...
function seekSingle(data: any, pathStr: string, result: AnyObject, key: any) {
  if (pathStr === '$' || pathStr === '@') {
    if (key.toString().indexOf('.*') >= 0 && data && typeof data === 'object') {
      ...
      Object.keys(data)
        .filter(function(outkey) { return outkey.indexOf(prefix) === 0; })
  ...
  if (key.toString().indexOf('.*') >= 0 && seek.length > 0) {
    ...
    if (base && typeof base === 'object') {
      Object.keys(base)
        .filter(function(outkey) { return outkey.indexOf(prefix2) === 0; })
        .forEach(function(outkey) { result[outkey] = base[outkey]; });
      return;
    }
  }
...
function seekArray(data: any, pathArr: any[], result: AnyObject, key: any) {
  ...
  if (seek.length && subpath) {
    ...
    if (Array.isArray(seek[0])) {
      seek[0].forEach(function(item: any, index: number) { walk(item, subpath, arr as any, index); });
    } else {
      seek.forEach(function(item: any, index: number) { walk(item, subpath, arr as any, index); });
    }
    ...
    if (subpath.ARRAY === true) {
      arr.forEach(function(obj, idx) {
        delete obj.ARRAY;
        const res: any[] = [];
        Object.keys(obj).forEach(function(i) { res[parseInt(i as any, 10)] = obj[i]; });
        (arr as any)[idx] = res;
      });
    }
    while (arr.indexOf(null) >= 0) { arr.splice(arr.indexOf(null), 1); }
    while (arr.indexOf(undefined) >= 0) { arr.splice(arr.indexOf(undefined), 1); }
  } else {
...
function iterateKeysObject(data: any, template: any, result: AnyObject, searchPath: string) {
  ...
  if (scriptMatch) {
    ...
    if (/\.\.$/.test(basePart)) {
      ...
      baseVals.forEach((v) => {
        if (Array.isArray(v)) { contexts.push(...v); }
      });
    } else {
      const baseVals = JSONPath({ path: basePart, json: data, wrap: true }) as any[];
      baseVals.forEach((v) => {
        if (Array.isArray(v)) { contexts.push(...v); } else { contexts.push(v); }
      });
    }
    contexts.forEach((ctx) => {
  const keyVal = evalExpression(expr, ctx, currentEnv());
      if (!keyVal) { return; }
  String(keyVal).split(/[,;\s]+/).forEach(function(split_key: string) {
        if (!result[split_key]) { result[split_key] = []; }
        const res: AnyObject = {};
...
function seekObject(data: any, pathObj: any, result: AnyObject, key?: any) {
  ...
  Object.keys(pathObj).forEach(function(name) { walk(data, pathObj[name], result, name); });
...
walk = function(data: any, path: any, result: AnyObject, key?: any) {
  let fn: any;
  switch (typeOf(path)) {
    case 'string': fn = seekSingle; break;
    case 'number': (result as any)[key] = path; break;
    case 'array': fn = seekArray; break;
    case 'object': fn = seekObject; break;
  }
  if (fn) { fn(data, path, result, key); }
};

export default function transform(data: any, path: any, env?: Record<string, any>) {
  const result: AnyObject = {};
  if (!env) { env = {}; } else { Object.freeze(env); }
  withEnv(env, function() { walk(data, path, result); });
  return result;
}
